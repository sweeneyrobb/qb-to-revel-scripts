...
Task transform -action {
    ...
    Where-Object { [int]$_."Qty 1" -gt 0 }

    $script:transformedData = $script:transformedData
...
Task transform-select -action {
    $script:transformedData = $script:transformedData | Select-Object -Property `
        ...
        @{ Name = "Qty";     Expression = { $_."Qty 1" } },
        @{ Name = "Reason";     Expression = { "Initial Import" } }
} -depends "transform"

Task export -action {
    $script:transformedData | Export-Csv ".\dist\cf-inventory-export-20160428.csv" -Force -NoTypeInformation
